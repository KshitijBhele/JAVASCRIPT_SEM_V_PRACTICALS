...
let jobs = [
    { id: 1, title: "Frontend Developer", company: "TechCorp", salary: 75000, category: "IT", experience: "Mid-level", location: "Remote", type: "Full-time" },
    { id: 2, title: "Backend Developer", company: "DataSystems", salary: 85000, category: "IT", experience: "Senior", location: "New York", type: "Full-time" },
    { id: 3, title: "UX Designer", company: "CreativeMinds", salary: 70000, category: "Design", experience: "Mid-level", location: "Remote", type: "Full-time" },
    { id: 4, title: "Data Analyst", company: "AnalyticsPro", salary: 65000, category: "Data", experience: "Junior", location: "Chicago", type: "Full-time" },
    { id: 5, title: "Project Manager", company: "ManageWell", salary: 90000, category: "Management", experience: "Senior", location: "Remote", type: "Full-time" }
];
...
document.getElementById('loadJobsFromJSON').addEventListener('click', function() {
    showNotification('Loading jobs from JSON file...');
    ...
    fetch('data.json')
        ...
        .then(data => {
            if (data.jobListings && data.jobListings.length > 0) {
                ...
                displayJobs();
                showNotification('Jobs loaded successfully from JSON!');
            } else {
                throw new Error('No job listings found in JSON');
            }
        })
        .catch(error => {
            console.error('Error loading jobs from JSON:', error);
            showNotification('Error loading jobs from JSON file', 'error');
        });
});
...
document.getElementById('loadSchedule').addEventListener('click', function() {
    showNotification('Loading interview schedule from JSON...');
    ...
    fetch('data.json')
        ...
        .then(data => {
            displaySchedule(data.interviewSchedule);
            showNotification('Schedule loaded successfully from JSON file');
        })
        .catch(error => {
            console.error('Error loading JSON:', error);
            showNotification('Error loading schedule data from JSON', 'error');
            
            const fallbackData = [
                { date: "2024-12-15", time: "10:00 AM", company: "TechCorp", position: "Frontend Developer", status: "Scheduled", location: "Remote", interviewer: "John Smith" },
                { date: "2024-12-18", time: "2:30 PM", company: "DataSystems", position: "Backend Developer", status: "Scheduled", location: "Office", interviewer: "Sarah Johnson" }
            ];
            displaySchedule(fallbackData);
        });
});
